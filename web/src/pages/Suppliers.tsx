...
import { Plus, Search, Edit2, Trash2, X, Truck, Phone, MapPin } from 'lucide-react';
...
export default function Suppliers() {
  ...
  const save = async () => {
    ...
    if (editing) await updateDoc(doc(db, 'suppliers', editing.id), form);
    else await addDoc(collection(db, 'suppliers'), form);
    setModal(false); load();
  };
  const del = async (item: any) => {
    if (!confirm(t('confirm_delete') + ' "' + item.name + '"?')) return;
    await deleteDoc(doc(db, 'suppliers', item.id)); load();
  };
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700">
              <h2 className="font-semibold text-white">{editing ? t('edit_supplier') : t('add_supplier')}</h2>
              <button onClick={() => setModal(false)} className="text-slate-400 hover:text-white"><X className="w-4 h-4" /></button>
            </div>
            <div className="p-5 space-y-3">
              {[
                { label: t('supplier_name'), key: 'name' },
